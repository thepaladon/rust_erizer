...
use jobsys::{JobInstance, JobScope};

use crate::{camera::Camera, triangle::Triangle, mesh::RenderMode};
...
impl SlicedBuffers {
    ...
    pub fn render(&mut self, camera: &Camera, render_type : &RenderMode, job_scope : &JobScope) {
        
        for tile in self.tiles.iter_mut() {
            JobInstance::create(job_scope, || {
                tile.render(&self.triangles, camera, render_type);
            });
        }
                ...
                let tile = &self.tiles[(x + y * self.size.x) as usize];
                    for t_x in 0..tile.size.x{
                        for t_y in 0..tile.size.y{
                            ...
                            let dst = (x * tile.size.x + t_x + (y * tile.size.y + t_y) * crate::WIDTH as i32) as usize;
