...
use glam::{IVec2, Vec2, Vec3, Vec4, Vec4Swizzles, UVec2};
...
impl Triangle {
    ...
    pub fn triangle_to_screen(triangle: &Triangle) -> Triangle {
        ...
        let viewport_size = {
            let viewport = VIEWPORT.read().unwrap(); 
            UVec2::new(viewport.width, viewport.height)
        };

        // screeen coordinates remapped to window
        let sc0 = glam::vec2(
            render_utils::map_to_range(ndc0.x, -1.0, 1.0, 0.0, viewport_size.x as f32),
            render_utils::map_to_range(-ndc0.y, -1.0, 1.0, 0.0, viewport_size.y as f32),
        );
        let sc1 = glam::vec2(
            render_utils::map_to_range(ndc1.x, -1.0, 1.0, 0.0, viewport_size.x as f32),
            render_utils::map_to_range(-ndc1.y, -1.0, 1.0, 0.0, viewport_size.y as f32),
        );
        let sc2 = glam::vec2(
            render_utils::map_to_range(ndc2.x, -1.0, 1.0, 0.0, viewport_size.x as f32),
            render_utils::map_to_range(-ndc2.y, -1.0, 1.0, 0.0, viewport_size.y as f32),
        );
    ...
    pub fn render_fragments(
        ...
        for idx_x in 0..size.x {
            for idx_y in 0..size.y {
                ...
                let render_type = RenderMode::new(*render_type);
    ...
    fn calc_aabb(&mut self, vertices: [Vec2; 3]) {
        ...
        taabb[1].x = taabb[1].x.min(viewport_size.x as f32 - 1.0);
        taabb[1].y = taabb[1].y.min(viewport_size.y as f32 - 1.0);
